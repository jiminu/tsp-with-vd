...
rg_BetaSpan::rg_BetaSpan(const rg_INT& numInterval)
...
rg_BetaSpan::rg_BetaSpan(const rg_BetaSpan& span)
...
rg_INT   rg_BetaSpan::getNumBetaInterval() const
{
...
rg_INT   rg_BetaSpan::getBoundingState(const rg_INT& i) const
{
...
rg_INT*  rg_BetaSpan::getBoundingStates() const
{
...
rg_REAL* rg_BetaSpan::getBetaIntervals() const
{
...
Interval_REAL rg_BetaSpan::getBetaInterval(const rg_INT& i) const
{
    if ( i>=0 && i<m_numBetaInterval ) {
...
rg_REAL rg_BetaSpan::getLowerValueOfInterval(const rg_INT& i) const
{
    if ( i>=0 && i<m_numBetaInterval ) {
...
rg_REAL rg_BetaSpan::getUpperValueOfInterval(const rg_INT& i) const
{
    if ( i>=0 && i<m_numBetaInterval ) {
...
rg_BOOL rg_BetaSpan::findBetaIntervalOfGivenState(const rg_INT& state, Interval_REAL& interval) const
{
    rg_BOOL isFound = rg_FALSE;

    for ( rg_INT i=0; i<m_numBetaInterval; i++ ) {
        if ( m_boundingState[i] == state ) {
            interval.setLowerAndUpperValue( m_betaInterval[i], m_betaInterval[i+1] );
            isFound = rg_TRUE;
            break;
        }
    }
...
Interval_REAL rg_BetaSpan::getExposureInterval() const
{
    return getBetaIntervalOfSingularRegularState();
}


Interval_REAL rg_BetaSpan::getBetaIntervalOfSingularRegularState() const
{
...
void rg_BetaSpan::setNumBetaInterVal(const rg_INT& numInterval)
{
    m_numBetaInterval = numInterval;
    if ( m_boundingState != rg_NULL ) {
        delete [] m_boundingState;
...
void rg_BetaSpan::setBetaInterval(const rg_INT& i, const rg_REAL& lower, const rg_REAL& upper)
{
    if ( i>=0 && i<m_numBetaInterval ) {
...
void rg_BetaSpan::setBetaInterval(const rg_INT& i, const Interval_REAL& interval)
{
    if ( i>=0 && i<m_numBetaInterval ) {
...
//void BetaSpanCore::setBetaInterval(const rg_INT& i, const rg_INT& boundingState, const rg_REAL& lower, const rg_REAL& upper)
//{
//    if ( i>=0 && i<m_numBetaInterval ) {
...
void rg_BetaSpan::setBetaInterval(const rg_INT& i, const rg_INT& boundingState, const Interval_REAL& interval)
{
    if ( i>=0 && i<m_numBetaInterval ) {
        m_boundingState[i]  = boundingState;
...
void rg_BetaSpan::shiftBetaSpan(const rg_REAL& delta)
{
    for (rg_INT i=0; i<=m_numBetaInterval; i++ ) 
	{
...
rg_BetaSpan& rg_BetaSpan::operator=(const rg_BetaSpan& span)
{
    if ( this == &span ) {
        return *this;
    }
...
rg_INT   rg_BetaSpan::findBoundingState(const rg_REAL& beta) const
{
...
rg_BetaSpan rg_BetaSpan::merge(const rg_BetaSpan& thatSpan) const
{
...
void     rg_BetaSpan::unify(const rg_BetaSpan& span)
{
    rg_BetaSpan newBetaSpan = merge(span);
...
void     rg_BetaSpan::report(ofstream& fout)
{
    if ( m_numBetaInterval <= 0 ) {
        return;
    }
