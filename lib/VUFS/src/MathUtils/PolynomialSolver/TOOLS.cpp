...
/*                if flag=rg_TRUE, it additionally computes the      */
/*                derivative df=P'(x0)                            */
/*                                                                */
/* Copyright:                                                     */
...
void hornc(dcomplex *pred,rg_INT nred,dcomplex x0,unsigned char flag) 
/*dcomplex      *pred,  coefficient vector of the polynomial           */
/*              x0;     root to be deflated                            */
/*rg_INT           nred;   the highest exponent of (deflated) polynomial  */
...
void horncd(dcomplex *pred,rg_INT nred,rg_REAL a,rg_REAL b)
/*dcomplex *pred;     coefficient vector of the polynomial           */
/*rg_REAL   a,         coefficients of the quadratic polynomial       */
/*         b;         x^2+ax+b                                       */
/*rg_INT      nred;      the highest exponent of (deflated) polynomial  */
{
     rg_INT i;        /* counter */
...
rg_INT poldef(dcomplex *pred,rg_INT nred,dcomplex *root,unsigned char flag)
/*dcomplex *pred,     coefficient vector of the polynomial           */
...
void fdvalue(dcomplex *p,rg_INT n,dcomplex *f,dcomplex *df,dcomplex x0,
             unsigned char flag) 
/*dcomplex      *p,     coefficient vector of the polynomial P(x)   */
/*              *f,     the result f=P(x0)                          */
/*              *df,    the result df=P'(x0), if flag=rg_TRUE          */
/*              x0;     polynomial will be computed at x0           */
/*rg_INT           n;      the highest exponent of p                   */
/*unsigned char flag;   flag==rg_TRUE => compute P'(x0)                */
{
     rg_INT      i;     /* counter                                     */
     dcomplex help1; /* help variable                               */ 

     *f  = p[n];
     if (flag==rg_TRUE) {              /* if flag=rg_TRUE, compute P(x0)  */
