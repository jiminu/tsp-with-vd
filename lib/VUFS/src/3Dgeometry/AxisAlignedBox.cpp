...
bool AxisAlignedBox::doesInclude( const rg_Point3D& pt ) const
{
    double x = pt.getX();
    double y = pt.getY();
    double z = pt.getZ();

    if ( x < m_minPoint.getX() )
        return false;
    if ( y < m_minPoint.getY() )
        return false;
    if ( z < m_minPoint.getZ() )
        return false;

    if ( x > m_maxPoint.getX() )
        return false;
    if ( y > m_maxPoint.getY() )
        return false;
    if ( z > m_maxPoint.getZ() )
        return false;
...
bool AxisAlignedBox::doesInclude( const Sphere& sphere ) const
{
    double x = sphere.getCenter().getX();
    double y = sphere.getCenter().getY();
    double z = sphere.getCenter().getZ();
    ...
    if ( x - r < m_minPoint.getX() )
        return false;
    if ( y - r < m_minPoint.getY() )
        return false;
    if ( z - r < m_minPoint.getZ() )
        return false;

    if ( x + r > m_maxPoint.getX() )
        return false;
    if ( y + r > m_maxPoint.getY() )
        return false;
    if ( z + r > m_maxPoint.getZ() )
        return false;
