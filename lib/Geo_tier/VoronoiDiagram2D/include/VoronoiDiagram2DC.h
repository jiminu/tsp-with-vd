...
const bool IS_BUCKET_USED = false;
//BUCKET -> Have error using insert or delete because don't update bucket during insert or delete.
...
public:
    ...
    void         clearGeneratorsWithoutDeletingDynamicMemoryOfTheGenerators();
	void         clearTopologyUnrelatedObjects();
    ...
    void         updateGeometry();
    void         updateGeometryOfInsertedGenerator(Generator2D*);
    void         updateGeometryOfDeletedGenerator(Generator2D* removedGenerator, list<VEdge2D*>& influencedEdges);
    
    Generator2D* deleteOneGenerator(Generator2D* generator);
    Generator2D* deleteOneGenerator(Generator2D* generator, list<VEdge2D*>& influencedEdges);
    Generator2D* deleteOneGenerator_deleteFromGeneratorList(Generator2D* generator);
    Generator2D* deleteOneGenerator_deleteFromGeneratorList(Generator2D* generator, list<VEdge2D*>& influencedEdges);
    void		 insertOneGenerator(Generator2D* generator);
    void         insertOneGenerator_addToGeneratorList(Generator2D* generator);
    
    virtual Generator2D*    insertDiskIntoVD( const rg_Circle2D& disk );
    virtual Generator2D*    insertDiskIntoVD( pair< rg_Circle2D, void* >& diskNUserDataPair);
    virtual Generator2D*    insertDiskIntoVD( const rg_Triplet< rg_Circle2D, int, void* > & diskAndIDAndUserData);
...
protected:
    ...
    void            copyTopologyUnrelatedObjects(const VoronoiDiagram2DC& VD2DC);
    void            getAllVEntities(const VoronoiDiagram2DC& VD2DC,
    ...
    void createNLinkEntities(const list<Generator2D*>& oldGenerators,
    ...
    void            createNLinkGenerators(const list<Generator2D*>& generators,  unordered_map<Generator2D*, Generator2D*>& generatorMapFromOldToNew) const;
	void            createNLinkFaces(     const list<VFace2D*>& faces,       unordered_map<VFace2D*, VFace2D*>& faceMapFromOldToNew) const;
	void            createNLinkEdges(     const list<VEdge2D*>& edges,       unordered_map<VEdge2D*, VEdge2D*>& edgeMapFromOldToNew) const;
	void            createNLinkVertices(   const list<VVertex2D*>& vertices,   unordered_map<VVertex2D*, VVertex2D*>& vertexMapFromOldToNew) const;

    void            insertCreatedEntitiesToMemberData(
	...
	void            insertCreatedPhantomGenerators( const list<Generator2D*>& oldPhantomGenerators, const unordered_map<Generator2D*, Generator2D*>& phantomGeneratorMapFromOldToNew);
    void            insertCreatedGenerators( const list<Generator2D*>& oldGenerators, const unordered_map<Generator2D*, Generator2D*>& generatorMapFromOldToNew);
	void            insertCreatedFaces(      const list<VFace2D*>&     oldFaces,      const unordered_map<VFace2D*,   VFace2D*>&       faceMapFromOldToNew);
	void            insertCreatedEdges(      const list<VEdge2D*>&     oldEdges,      const unordered_map<VEdge2D*,   VEdge2D*>&       edgeMapFromOldToNew);
	void            insertCreatedVertices(   const list<VVertex2D*>&   oldVertices,   const unordered_map<VVertex2D*, VVertex2D*>&     vertexMapFromOldToNew);
    ...
    void            connectCreatedEntitiesInFaces(
                        const list<VFace2D*>& oldFaces,
                        const unordered_map<Generator2D*, Generator2D*>& generatorMapFromOldToNew,
                        const unordered_map<VFace2D*, VFace2D*>& faceMapFromOldToNew,
                        const unordered_map<VEdge2D*, VEdge2D*>& edgeMapFromOldToNew) const;
   
    void            connectCreatedEntitiesInEdges(
                        const list<VEdge2D*>& oldEdges,
                        const unordered_map<VFace2D*, VFace2D*>& faceMapFromOldToNew,
                        const unordered_map<VEdge2D*, VEdge2D*>& edgeMapFromOldToNew,
                        const unordered_map<VVertex2D*, VVertex2D*>& vertexMapFromOldToNew) const;

    void            connectCreatedEntitiesInVertices(
                        const list<VVertex2D*>& oldVertices,
                        const unordered_map<VEdge2D*, VEdge2D*>& edgeMapFromOldToNew,
                        const unordered_map<VVertex2D*, VVertex2D*>& vertexMapFromOldToNew) const;
...
public:
    virtual void updateEdge( VEdge2D* target );
protected:
    ...
    static bool  compareCircleAscendingOrder( const rg_Circle2D& circle1, const rg_Circle2D& circle2 );
