...
BetaSpan VertexBU2D::getBetaSpan() const
{
    return m_betaSpan;
}
...
rg_BOOL VertexBU2D::isMemberOf( const FaceBU2D* const face ) const
{
    rg_BOOL isMemberOfFace = false;

    rg_dList<FaceBU2D*> incidentFaces;
    getIncidentFaces( incidentFaces );

    incidentFaces.reset4Loop();
    while( incidentFaces.setNext4Loop() ) {
        FaceBU2D* currFace = incidentFaces.getEntity();
...
rg_BOOL VertexBU2D::isMemberOf( const EdgeBU2D* const edge ) const
{
    rg_BOOL isMemberOfEdge = false;
...
rg_BOOL VertexBU2D::isAdjacentTo( const VertexBU2D* const vertex ) const
{
    rg_BOOL isAdjacentToVertex = false;

    rg_dList<VertexBU2D*> adjacentVertices;
    getNeighborVertices( adjacentVertices );

    adjacentVertices.reset4Loop();
    while( adjacentVertices.setNext4Loop() ) {
        VertexBU2D* currVertex = adjacentVertices.getEntity();
