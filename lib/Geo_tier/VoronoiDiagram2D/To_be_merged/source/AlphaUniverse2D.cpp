#include "AlphaUniverse2D.h"
using namespace BULL2D::GeometryTier;

#include <stdlib.h> 


AlphaUniverse2D::AlphaUniverse2D()
{
...
void AlphaUniverse2D::construct(DelaunayTriangulation2D_GEO& DT2D)
{
    convert( DT2D );
...
// Query functions
void  AlphaUniverse2D::huntFacesInBetaComplex(    const rg_REAL& beta, rg_INT& startIndex, rg_INT& endIndex ) const
{
    startIndex = 0;
    endIndex   = 0;
...
void  AlphaUniverse2D::huntEdgesInBetaComplex(    const rg_REAL& beta, rg_INT& startIndex, rg_INT& endIndex ) const
{
    startIndex = 0;
    endIndex   = 0;
...
void  AlphaUniverse2D::huntVerticesInBetaComplex( const rg_REAL& beta, rg_INT& startIndex, rg_INT& endIndex ) const
{
    startIndex = 0;
...
void  AlphaUniverse2D::huntFacesInBetaComplex(    const rg_REAL& beta, rg_dList<FaceBU2D*>&   betaFaces ) const
{
...
void  AlphaUniverse2D::huntEdgesInBetaComplex(    const rg_REAL& beta, rg_dList<EdgeBU2D*>&   betaEdges ) const
{
...
void  AlphaUniverse2D::huntVerticesInBetaComplex( const rg_REAL& beta, rg_dList<VertexBU2D*>& betaVertices ) const
{
...
void  AlphaUniverse2D::huntEdgesInBetaShape(    const rg_REAL& beta, rg_dList<EdgeBU2D*>&   betaEdges ) const
{
    for ( rg_INT i=0; i < m_numFiniteEdges; i++ ) {
...
void  AlphaUniverse2D::huntVerticesInBetaShape( const rg_REAL& beta, rg_dList<VertexBU2D*>& betaVertices ) const
{
    for ( rg_INT i=0; i < m_numFiniteVertices; i++ ) {
...
void    AlphaUniverse2D::reportBetaUniverse2D(const string& reportFilename)
{
    ofstream fout;
...
void AlphaUniverse2D::convert(DelaunayTriangulation2D_GEO& DT2D)
{
    ...
    createAndInitializeFacesInBU2D( DT2D, mappingTableFromFaceInQTToFaceInBU );
    createAndInitializeVerticesInBU2D( DT2D, mappingTableFromVtxInQTToVtxInBU );

    rg_INT i=0;
	rg_INT              numFacesInQT2D = DT2D.getNumFaces();
	rg_dList<FaceQT2D>& facesInQT2D    = DT2D.getFaces();
...
void AlphaUniverse2D::computeBetaSpan()
{
...
void AlphaUniverse2D::generateSortedFiniteSimplexes()
{
...
void AlphaUniverse2D::generateSortedFiniteFaces()
{
    m_numFiniteFaces = 0;
    ...
    qsort( (void *) m_sortedFiniteFaces, m_numFiniteFaces, sizeof(FaceBU2D*), compareAlphaSpanOfFaces);
}



void AlphaUniverse2D::generateSortedFiniteEdges()
{
    m_numFiniteEdges = 0;
    ...
    qsort( (void *) m_sortedFiniteEdges, m_numFiniteEdges, sizeof(EdgeBU2D*), compareAlphaSpanOfEdges);
}



void AlphaUniverse2D::generateSortedFiniteVertices()
{
    m_numFiniteVertices = 0;
    ...
    qsort( (void *) m_sortedFiniteVertices, m_numFiniteVertices, sizeof(VertexBU2D*), compareAlphaSpanOfVertices);
}
...
void AlphaUniverse2D::createAndInitializeFacesInBU2D( DelaunayTriangulation2D_GEO& DT2D,    map<FaceSCDS*, FaceBU2D*>&     mappingTableFromFaceInQTToFaceInBU )
{	
	rg_dList<FaceQT2D>& facesInQT2D    = DT2D.getFaces();
...
void AlphaUniverse2D::createAndInitializeVerticesInBU2D( DelaunayTriangulation2D_GEO& DT2D, map<VertexSCDS*, VertexBU2D*>& mappingTableFromVtxInQTToVtxInBU )
{	
	rg_dList<VertexQT2D>& verticesInQT2D = DT2D.getVertices();
...
int BULL2D::GeometryTier::compareAlphaSpanOfFaces(const void* ts1, const void* ts2)
{
...
int BULL2D::GeometryTier::compareAlphaSpanOfEdges(const void* ts1, const void* ts2)
{
...
int BULL2D::GeometryTier::compareAlphaSpanOfVertices(const void* ts1, const void* ts2)
{
...
void AlphaUniverse2D::computeBetaInterval()
{
...
void AlphaUniverse2D::rearrangeSimplexIDs()
{
    rearrangeFaceIDs();
    rearrangeEdgeIDs();
    rearrangeVertexIDs();
...
void AlphaUniverse2D::rearrangeFaceIDs()
{
	m_faces.reset4Loop();
	while( m_faces.setNext4Loop() )  {
...
void AlphaUniverse2D::rearrangeEdgeIDs()
{
	m_edges.reset4Loop();
	while( m_edges.setNext4Loop() )  {
...
void AlphaUniverse2D::rearrangeVertexIDs()
{
	m_vertices.reset4Loop();
	while( m_vertices.setNext4Loop() )  {
