...
VEdge2D::VEdge2D( const VEdge2D& edge )
{
    ...
    m_tangentCircle                     = edge.m_tangentCircle;
    m_radiusIntervalOfTangentCircles[0] = edge.m_radiusIntervalOfTangentCircles[0];
	m_radiusIntervalOfTangentCircles[1] = edge.m_radiusIntervalOfTangentCircles[1];

    m_geometry = edge.m_geometry;
}
...
void V::GeometryTier::VEdge2D::setGeometry(const rg_RQBzCurve2D& geometry)
{
    m_geometry = geometry;
}
